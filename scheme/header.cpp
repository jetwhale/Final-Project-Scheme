...
u64 prim__43(u64 a, u64 b) // +
{
    ...
    if ((DECODE_INT(b)) < 1){
        if (-2147483648 - (DECODE_INT(b)) >= (DECODE_INT(a))){
            printf("Integer Underflow Error\n");
            exit(1);
        }
    }
    else{
        if (2147483647 - (DECODE_INT(b)) <= (DECODE_INT(a))){
...
u64 prim__42(u64 a, u64 b) // *
{
    ...
    unsigned long long prod = (unsigned long long)(DECODE_INT(a) * DECODE_INT(b));
    if (prod > 2147483647){
        printf("Integer Overflow Error\n");
        exit(1);
    }
